...
// Generated by REAPER v6.78/win64
...
REAPERAPI_DEF //==============================================
    ...
    // Calculates loudness statistics of media via dry run render. Statistics
    // will be displayed to the user; call
    // GetSetProjectInfo_String("RENDER_STATS") to retrieve via API. Returns 1
    // if loudness was calculated successfully, -1 if user canceled the dry run
    // render.

    int (*CalcMediaSrcLoudness)(PCM_source* mediasource);
#endif

#if defined(REAPERAPI_WANT_CalculateNormalization) || \
    !defined(REAPERAPI_MINIMAL)
REAPERAPI_DEF //==============================================
              // CalculateNormalization
    // Calculate normalize adjustment for source media. normalizeTo: 0=LUFS-I,
    // 1=RMS-I, 2=peak, 3=true peak, 4=LUFS-M max, 5=LUFS-S max.
    // normalizeTarget: dBFS or LUFS value. normalizeStart, normalizeEnd: time
    // bounds within source media for normalization calculation. If
    // normalizationStart=0 and normalizationEnd=0, the full duration of the
    // media will be used for the calculation.
...
REAPERAPI_DEF //==============================================
    ...
    // added delay to output in 100ths of a millisecond.
        ...
        midi_Output* (
            *CreateMIDIOutput)(int dev, bool streamMode, int* msoffset100);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // seconds unless qn is set.
        ...
        MediaItem* (*CreateNewMIDIItemInProj)(
            MediaTrack* track,
            double starttime,
            double endtime,
...
REAPERAPI_DEF //==============================================
    ...
    int (*CSurf_OnInputMonitorChangeEx)(
        MediaTrack* trackid,
        int monitor,
        bool allowgang);
#endif
...
#if defined(REAPERAPI_WANT_CSurf_OnOscControlMessage2) || \
    !defined(REAPERAPI_MINIMAL)
REAPERAPI_DEF //==============================================
              // CSurf_OnOscControlMessage2

    void (*CSurf_OnOscControlMessage2)(
        const char* msg,
        const float* arg,
        const char* argstr);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    double (*CSurf_OnRecvVolumeChange)(
        MediaTrack* trackid,
        int recv_index,
        double volume,
        bool relative);
...
REAPERAPI_DEF //==============================================
    ...
    double (*CSurf_OnSendVolumeChange)(
        MediaTrack* trackid,
        int send_index,
        double volume,
        bool relative);
...
REAPERAPI_DEF //==============================================
    ...
    double (
        *CSurf_OnWidthChange)(MediaTrack* trackid, double width, bool relative);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*DeleteEnvelopePointRangeEx)(
        TrackEnvelope* envelope,
        int autoitem_idx,
        double time_start,
        double time_end);
...
REAPERAPI_DEF //==============================================
    ...
    // rendered within this region.
        ...
        MediaTrack* (*EnumRegionRenderMatrix)(
            ReaProject* proj,
            int regionindex,
            int rendertrack);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*EnumTrackMIDIProgramNames)(
        int track,
        int programNumber,
        char* programName,
        int programName_sz);
...
REAPERAPI_DEF //==============================================
    ...
    bool (*EnumTrackMIDIProgramNamesEx)(
        ReaProject* proj,
        MediaTrack* track,
        int programNumber,
        char* programName,
...
REAPERAPI_DEF //==============================================
    ...
    // If take envelope, gets the take from the envelope. If FX, indexOut set to
    // FX index, index2Out set to parameter index, otherwise -1.

        MediaItem_Take* (*Envelope_GetParentTake)(
            TrackEnvelope* env,
            int* indexOut,
            int* index2Out);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // If track envelope, gets the track from the envelope. If FX, indexOut set
    // to FX index, index2Out set to parameter index, otherwise -1.

        MediaTrack* (*Envelope_GetParentTrack)(
            TrackEnvelope* env,
            int* indexOut,
            int* index2Out);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (
        *get_config_var_string)(const char* name, char* bufOut, int bufOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // height of envelope I_TCPY_USED : int : Y offset of envelope relative to
    // parent track, exclusive of padding I_TCPH_USED : int : visible height of
    // envelope, exclusive of padding P_TRACK : MediaTrack * : parent track
    // pointer (if any) P_DESTTRACK : MediaTrack * : destination track pointer,
    // if on a send P_ITEM : MediaItem * : parent item pointer (if any) P_TAKE :
    // MediaItem_Take * : parent take pointer (if any) I_SEND_IDX : int :
    // 1-based index of send in P_TRACK, or 0 if not a send I_HWOUT_IDX : int :
    // 1-based index of hardware output in P_TRACK or 0 if not a hardware output
...
REAPERAPI_DEF //==============================================
    ...
    // written, &4 if the envelope recently had an effective automation mode
    // change
...
REAPERAPI_DEF //==============================================
    ...
    // returns approximate input level if available, 0-511 mono inputs, |1024
    // for stereo pairs, 4096+devidx*32 for MIDI devices
...
REAPERAPI_DEF //==============================================
    ...
    // specified, returns the take hit. See GetThingFromPoint.

        MediaItem* (*GetItemFromPoint)(
            int screen_x,
            int screen_y,
            bool allow_locked,
...
REAPERAPI_DEF //==============================================
    ...
    void (*GetLastMarkerAndCurRegion)(
        ReaProject* proj,
        double time,
        int* markeridxOut,
        int* regionidxOut);
...
REAPERAPI_DEF //==============================================
    ...
    // returns &1 if the master track is visible in the TCP, &2 if NOT visible
    // in the mixer. See SetMasterTrackVisibility.
...
REAPERAPI_DEF //==============================================
    ...
    // : int * : group ID, 0=no group I_LASTY : int * : Y-position (relative to
    // top of track) in pixels (read-only) I_LASTH : int * : height in pixels
    // (read-only) I_CUSTOMCOLOR : int * : custom color, OS dependent
    // color|0x1000000 (i.e. ColorToNative(r,g,b)|0x1000000). If you do not
    // |0x1000000, then it will not be used, but will store the color I_CURTAKE
    // : int * : active take number IP_ITEMNUMBER : int : item number on this
    // track (read-only, returns the item number directly) F_FREEMODE_Y : float
    // * : free item positioning Y-position, 0=top of track, 1=bottom of track
    // (will never be 1) F_FREEMODE_H : float * : free item positioning height,
    // 0=no height, 1=full height of track (will never be 0) P_TRACK :
    // MediaTrack * : (read-only)
    //
...
REAPERAPI_DEF //==============================================
    ...
    MediaItem_Take* (
        *GetMediaItemTakeByGUID)(ReaProject* project, const GUID* guid);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // : bool * : preserve pitch when changing playback rate I_LASTY : int * :
    // Y-position (relative to top of track) in pixels (read-only) I_LASTH : int
    // * : height in pixels (read-only) I_CHANMODE : int * : channel mode,
    // 0=normal, 1=reverse stereo, 2=downmix, 3=left, 4=right I_PITCHMODE : int
    // * : pitch shifter mode, -1=projext default, otherwise high 2
    // bytes=shifter, low 2 bytes=parameter I_CUSTOMCOLOR : int * : custom
    // color, OS dependent color|0x1000000 (i.e.
    // ColorToNative(r,g,b)|0x1000000). If you do not |0x1000000, then it will
    // not be used, but will store the color IP_TAKENUMBER : int : take number
...
REAPERAPI_DEF //==============================================
    ...
    // Copies the media source filename to filenamebuf. Note that in-project
    // MIDI media sources have no associated filename. See GetMediaSourceParent.

    void (*GetMediaSourceFileName)(
        PCM_source* source,
        char* filenamebufOut,
...
REAPERAPI_DEF //==============================================
    ...
    // place, 5=safe soloed, 6=safe soloed in place B_SOLO_DEFEAT : bool * :
    // when set, if anything else is soloed and this track is not muted, this
    // track acts soloed I_FXEN : int * : fx enabled, 0=bypassed, !0=fx active
    // I_RECARM : int * : record armed, 0=not record armed, 1=record armed
    // I_RECINPUT : int * : record input, <0=no input. if 4096 set, input is
    // MIDI and low 5 bits represent channel (0=all, 1-16=only chan), next 6
    // bits represent physical input (63=all, 62=VKB). If 4096 is not set, low
    // 10 bits (0..1023) are input start channel (ReaRoute/Loopback start at
    // 512). If 2048 is set, input is multichannel input (using track channel
    // count), or if 1024 is set, input is stereo input, otherwise input is
    // mono. I_RECMODE : int * : record mode, 0=input, 1=stereo out, 2=none,
    // 3=stereo out w/latency compensation, 4=midi output, 5=mono out, 6=mono
    // out w/ latency compensation, 7=midi overdub, 8=midi replace
    // I_RECMODE_FLAGS : int * : record mode flags, &3=output recording mode
    // (0=post fader, 1=pre-fx, 2=post-fx/pre-fader) I_RECMON : int * : record
    // monitoring, 0=off, 1=normal, 2=not when playing (tape style)
    // I_RECMONITEMS : int * : monitor items while recording, 0=off, 1=on
    // B_AUTO_RECARM : bool * : automatically set record arm when selected (does
    // not immediately affect recarm state, script should set directly if
    // desired) I_VUMODE : int * : track vu mode, &1:disabled, &30==0:stereo
    // peaks, &30==2:multichannel peaks, &30==4:stereo RMS, &30==8:combined RMS,
    // &30==12:LUFS-M, &30==16:LUFS-S (readout=max), &30==20:LUFS-S
    // (readout=current), &32:LUFS calculation on channels 1+2 only I_AUTOMODE :
    // int * : track automation mode, 0=trim/off, 1=read, 2=touch, 3=write,
    // 4=latch I_NCHAN : int * : number of track channels, 2-64, even numbers
    // only I_SELECTED : int * : track selected, 0=unselected, 1=selected I_WNDH
    ...
    // int * : current MCP X-position in pixels relative to mixer container
    // (read-only) I_MCPY : int * : current MCP Y-position in pixels relative to
    // mixer container (read-only) I_MCPW : int * : current MCP width in pixels
    // (read-only) I_MCPH : int * : current MCP height in pixels (read-only)
    ...
    // ColorToNative(r,g,b)|0x1000000). If you do not |0x1000000, then it will
    // not be used, but will store the color I_HEIGHTOVERRIDE : int * : custom
    ...
    // 0.5=-6dB, 0.707..=-3dB, 1=+0dB, 1.414..=-3dB with gain compensation,
    // 2=-6dB with gain compensation, etc I_PANLAW_FLAGS : int * : pan law
    // flags, 0=sine taper, 1=hybrid taper with deprecated behavior when gain
    // compensation enabled, 2=linear taper, 3=hybrid taper P_ENV:<envchunkname
    // or P_ENV:{GUID... : TrackEnvelope * : (read-only) chunkname can be
    // <VOLENV, <PANENV, etc; GUID is the stringified envelope GUID.
    // B_SHOWINMIXER : bool * : track control panel visible in mixer (do not use
    // on master track) B_SHOWINTCP : bool * : track control panel visible in
    // arrange view (do not use on master track) B_MAINSEND : bool * : track
    // sends audio to parent C_MAINSEND_OFFS : char * : channel offset of track
    // send to parent C_MAINSEND_NCH : char * : channel count of track send to
    // parent (0=use all child track channels, 1=use one channel only)
    // I_FREEMODE : int * : 1=track free item positioning enabled, 2=track fixed
    // lanes enabled (call UpdateTimeline() after changing) C_BEATATTACHMODE :
    // char * : track timebase, -1=project default, 0=time, 1=beats (position,
    // length, rate), 2=beats (position only) F_MCP_FXSEND_SCALE : float * :
    ...
    // (0=minimum allowed, 1=maximum allowed) F_MCP_SENDRGN_SCALE : float * :
    // scale of send area as proportion of the fx+send total area (0=minimum
    // allowed, 1=maximum allowed) F_TCP_FXPARM_SCALE : float * : scale of TCP
    // parameter area when TCP FX are embedded (0=min allowed, default, 1=max
    // allowed) I_PLAY_OFFSET_FLAG : int * : track media playback offset state,
    // &1=bypassed, &2=offset value is measured in samples (otherwise measured
    // in seconds) D_PLAY_OFFSET : double * : track media playback offset, units
    // depend on I_PLAY_OFFSET_FLAG P_PARTRACK : MediaTrack * : parent track
    // (read-only) P_PROJECT : ReaProject * : parent project (read-only)
    //
...
REAPERAPI_DEF //==============================================
    ...
    // command ID, or the custom action ID string. Note: the action string may
    // have a space and 'c' or 'm' appended to it to specify command ID vs mouse
    // modifier ID. See SetMouseModifier for more information.
    //

    void (*GetMouseModifier)(
        const char* context,
        int modifier_flag,
        char* actionOut,
        int actionOut_sz);
...
REAPERAPI_DEF //==============================================
    ...
    double (*GetSetAutomationItemInfo)(
        TrackEnvelope* env,
        int autoitem_idx,
        const char* desc,
        double value,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*GetSetAutomationItemInfo_String)(
        TrackEnvelope* env,
        int autoitem_idx,
        const char* desc,
...
REAPERAPI_DEF //==============================================
    ...
    // GUID : GUID * : 16-byte GUID, can query only, not set. If using a
    // _String() function, GUID is a string {xyz-...}.
    //

    bool (*GetSetEnvelopeInfo_String)(
        TrackEnvelope* env,
        const char* parmname,
        char* stringNeedBig,
        bool setNewValue);
...
REAPERAPI_DEF //==============================================
    ...
    bool (
        *GetSetItemState2)(MediaItem* item, char* str, int str_sz, bool isundo);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // * : free item positioning Y-position, 0=top of track, 1=bottom of track
    // (will never be 1) F_FREEMODE_H : float * : free item positioning height,
    // 0=no height, 1=full height of track (will never be 0) P_NOTES : char * :
    // item note text (do not write to returned pointer, use setNewValue to
    // update) P_EXT:xyz : char * : extension-specific persistent data GUID :
    // GUID * : 16-byte GUID, can query or update. If using a _String()
    // function, GUID is a string {xyz-...}.
    //

    void* (*GetSetMediaItemInfo)(
        MediaItem* item,
        const char* parmname,
        void* setNewValue);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*GetSetMediaItemInfo_String)(
        MediaItem* item,
        const char* parmname,
        char* stringNeedBig,
        bool setNewValue);
...
REAPERAPI_DEF //==============================================
    ...
    // changing playback rate I_LASTY : int * : Y-position (relative to top of
    // track) in pixels (read-only) I_LASTH : int * : height in pixels
    // (read-only) I_CHANMODE : int * : channel mode, 0=normal, 1=reverse
    // stereo, 2=downmix, 3=left, 4=right I_PITCHMODE : int * : pitch shifter
    // mode, -1=projext default, otherwise high 2 bytes=shifter, low 2
    // bytes=parameter I_CUSTOMCOLOR : int * : custom color, OS dependent
    // color|0x1000000 (i.e. ColorToNative(r,g,b)|0x1000000). If you do not
    // |0x1000000, then it will not be used, but will store the color
    // IP_TAKENUMBER : int : take number (read-only, returns the take number
    // directly)
    //

    void* (*GetSetMediaItemTakeInfo)(
        MediaItem_Take* tk,
        const char* parmname,
        void* setNewValue);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // space-separated triples of start time, end time, and envelope GUID
    // string.
    //   Example: "0.0 1.0 \"\" 0.0 1.0 "{xyz-...}"
    // P_RAZOREDITS_EXT : const char * : list of razor edit areas, as
    // comma-separated sets of space-separated tuples of start time, end time,
    // optional envelope GUID string, optional fixed/fipm top y-position,
    // optional fixed/fipm bottom y-position.
    //   Example: "0.0 1.0,0.0 1.0 "{xyz-...}",1.0 2.0 "" 0.25 0.5"
    // P_TCP_LAYOUT : const char * : layout name
    // P_EXT:xyz : char * : extension-specific persistent data
    // P_UI_RECT:tcp.mute : char * : read-only, allows querying screen position
    // + size of track WALTER elements (tcp.size queries screen position and
    // size of entire TCP, etc). GUID : GUID * : 16-byte GUID, can query or
    // update. If using a _String() function, GUID is a string {xyz-...}. B_MUTE
    // : bool * : muted B_PHASE : bool * : track phase inverted
    // B_RECMON_IN_EFFECT : bool * : record monitoring in effect (current
    // audio-thread playback state, read-only) IP_TRACKNUMBER : int : track
    // number 1-based, 0=not found, -1=master track (read-only, returns the int
    // directly) I_SOLO : int * : soloed, 0=not soloed, 1=soloed, 2=soloed in
    ...
    // &1=bypassed, &2=offset value is measured in samples (otherwise measured
    // in seconds) D_PLAY_OFFSET : double * : track media playback offset, units
    // depend on I_PLAY_OFFSET_FLAG
    //

    void* (*GetSetMediaTrackInfo)(
        MediaTrack* tr,
        const char* parmname,
        void* setNewValue);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // project regions, 6=all project markers, 7=selected project markers
    ...
    // markers/regions, &16=selected media items, &32=selected project
    // markers/regions RENDER_TAILMS : tail length in ms to render (only used if
    ...
    // shaping on stems RENDER_NORMALIZE: &1=enable, (&14==0)=LUFS-I,
    // (&14==2)=RMS, (&14==4)=peak, (&14==6)=true peak, (&14==8)=LUFS-M max,
    // (&14==10)=LUFS-S max, &32=normalize stems to common gain based on master,
    // &64=enable brickwall limit, &128=brickwall limit true peak, &256=only
    // normalize files that are too loud, &512=apply fade-in, &1024=apply
    // fade-out RENDER_NORMALIZE_TARGET: render normalization target as
    // amplitude, so 0.5 means -6.02dB, 0.25 means -12.04dB, etc
    // RENDER_BRICKWALL: render brickwall limit as amplitude, so 0.5 means
    // -6.02dB, 0.25 means -12.04dB, etc RENDER_FADEIN: render fade-in (0.001
    // means 1 ms, requires RENDER_NORMALIZE&512) RENDER_FADEOUT: render
    // fade-out (0.001 means 1 ms, requires RENDER_NORMALIZE&1024)
...
REAPERAPI_DEF //==============================================
    ...
    // necessarily the displayed number (read-only)
    // MARKER_INDEX_FROM_GUID:{GUID} : get the GUID index of the marker or
    // region with GUID {GUID} (read-only) OPENCOPY_CFGIDX : integer for the
    // configuration of format to use when creating copies/applying FX. 0=wave
    // (auto-depth), 1=APPLYFX_FORMAT, 2=RECORD_FORMAT RECORD_PATH : recording
    // directory -- may be blank or a relative path, to get the effective path
    // see GetProjectPathEx() RECORD_PATH_SECONDARY : secondary recording
    // directory RECORD_FORMAT : base64-encoded sink configuration (see project
    // files, etc). Callers can also pass a simple 4-byte string
    // (non-base64-encoded), e.g. "evaw" or "l3pm", to use default settings for
    // that sink type. APPLYFX_FORMAT : base64-encoded sink configuration (see
    // project files, etc). Used only if RECFMT_OPENCOPY is set to 1. Callers
    // can also pass a simple 4-byte string (non-base64-encoded), e.g. "evaw" or
    // "l3pm", to use default settings for that sink type. RENDER_FILE : render
    // directory RENDER_PATTERN : render file name (may contain wildcards)
    // RENDER_METADATA : get or set the metadata saved with the project (not
    // metadata embedded in project media). Example, ID3 album name metadata:
    // valuestr="ID3:TALB" to get, valuestr="ID3:TALB|my album name" to set.
    // Call with valuestr="" and is_set=false to get a semicolon-separated list
    // of defined project metadata identifiers. RENDER_TARGETS : semicolon
    // separated list of files that would be written if the project is rendered
    // using the most recent render settings RENDER_STATS : (read-only)
    // semicolon separated list of statistics for the most recently rendered
    // files. call with valuestr="XXX" to run an action (for example,
    // "42437"=dry run render selected items) before returning statistics.
    ...
    // RENDER_FORMAT2 : base64-encoded secondary sink configuration. Callers can
    // also pass a simple 4-byte string (non-base64-encoded), e.g. "evaw" or
    // "l3pm", to use default settings for that sink type, or "" to disable
    // secondary render.
    //

    bool (*GetSetProjectInfo_String)(
        ReaProject* project,
        const char* desc,
        char* valuestrNeedBig,
        bool is_set);
...
REAPERAPI_DEF //==============================================
    ...
    // Group can be one of: MEDIA_EDIT_LEAD MEDIA_EDIT_FOLLOW VOLUME_LEAD
    // VOLUME_FOLLOW
    // VOLUME_VCA_LEAD
    // VOLUME_VCA_FOLLOW
    // PAN_LEAD
    ...
    unsigned int (*GetSetTrackGroupMembership)(
        MediaTrack* tr,
...
REAPERAPI_DEF //==============================================
    ...
    unsigned int (*GetSetTrackGroupMembershipHigh)(
        MediaTrack* tr,
        const char* groupname,
        unsigned int setmask,
        unsigned int setvalue);
...
REAPERAPI_DEF //==============================================
    ...
    const char* (*GetSetTrackMIDISupportFile)(
        ReaProject* proj,
        MediaTrack* track,
        int which,
        const char* filename);
...
REAPERAPI_DEF //==============================================
    ...
    //  NULL)
    // parameter names:
    // P_DESTTRACK : MediaTrack * : destination track, only applies for
    // sends/recvs (read-only) P_SRCTRACK : MediaTrack * : source track, only
    // applies for sends/recvs (read-only) P_ENV:<envchunkname : TrackEnvelope *
    // : call with :<VOLENV, :<PANENV, etc appended (read-only) P_EXT:xyz : char
    // * : extension-specific persistent data B_MUTE : bool * B_PHASE : bool * :
    // true to flip phase B_MONO : bool * D_VOL : double * : 1.0 = +0dB etc
    // D_PAN : double * : -1..+1
    ...
    // 1-16=chan See CreateTrackSend, RemoveTrackSend.

    void* (*GetSetTrackSendInfo)(
        MediaTrack* tr,
        int category,
        int sendidx,
        const char* parmname,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*GetSetTrackSendInfo_String)(
        MediaTrack* tr,
        int category,
        int sendidx,
        const char* parmname,
...
REAPERAPI_DEF //==============================================
    ...
    TrackEnvelope* (
        *GetTakeEnvelopeByName)(MediaItem_Take* take, const char* envname);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // marker not valid. posOut will be set to position in item,
    // srcposOutOptional will be set to source media position. Returns index. if
    // input index is -1, the following marker is found using position (or
    // source position if position is -1). If position/source position are used
    // to find marker position, their values are not updated.

    int (*GetTakeStretchMarker)(
        MediaItem_Take* take,
        int idx,
        double* posOut,
...
REAPERAPI_DEF //==============================================
    ...
    // such as "arrange", "fx_chain", "fx_0" (first FX in chain, floating). If a
    // track panel is hit, string will begin with "tcp" or "mcp" or "tcp.mute"
    // etc (future versions may append additional information). May return NULL
    // with valid info string to indicate non-track thing.

        MediaTrack* (*GetThingFromPoint)(
            int screen_x,
            int screen_y,
            char* infoOut,
            int infoOut_sz);
...
REAPERAPI_DEF //==============================================
    ...
    int (*GetToggleCommandStateThroughHooks)(
        KbdSectionInfo* section,
        int command_id);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // "<VOLENV", or GUID string, like "{B577250D-146F-B544-9B34-F24FBE488F1F}".
    //

        TrackEnvelope* (*GetTrackEnvelopeByChunkName)(
            MediaTrack* tr,
...
REAPERAPI_DEF //==============================================
    ...
    TrackEnvelope* (
        *GetTrackEnvelopeByName)(MediaTrack* track, const char* envname);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // an envelope, 2 if it is likely a track FX. See GetThingFromPoint.

        MediaTrack* (*GetTrackFromPoint)(
            int screen_x,
            int screen_y,
            int* infoOutOptional);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // "1.1.2\tLyric for measure 1 beat 2\t2.1.1\tLyric for measure 2 beat 1	").
    // See SetTrackMIDILyrics

    bool (*GetTrackMIDILyrics)(
        MediaTrack* track,
        int flag,
...
REAPERAPI_DEF //==============================================
    ...
    const char* (*GetTrackMIDINoteNameEx)(
        ReaProject* proj,
        MediaTrack* track,
        int pitch,
        int chan);
...
REAPERAPI_DEF //==============================================
    ...
    // otherwise stereo pair, hwout:&512=rearoute I_MIDIFLAGS : int * : low 5
    // bits=source channel 0=all, 1-16, next 5 bits=dest channel, 0=orig,
    // 1-16=chan P_DESTTRACK : MediaTrack * : destination track, only applies
    // for sends/recvs (read-only) P_SRCTRACK : MediaTrack * : source track,
    // only applies for sends/recvs (read-only) P_ENV:<envchunkname :
...
REAPERAPI_DEF //==============================================
    ...
    bool (
        *GetTrackSendUIMute)(MediaTrack* track, int send_index, bool* muteOut);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*GetUnderrunTime)(
        unsigned int* audio_xrunOut,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*GetUserFileNameForRead)(
        char* filenameNeed4096,
        const char* title,
        const char* defext);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // items, &512=use high word as absolute track index if mode&3==0 or
    // mode&2048, &1024=insert into reasamplomatic on a new track (add 1 to
    // insert on last selected track), &2048=insert into open reasamplomatic
    // instance (add 512 to use high word as absolute track index), &4096=move
    // to source preferred position (BWF start offset), &8192=reverse
...
REAPERAPI_DEF //==============================================
    ...
    bool (*kbd_processMidiEventActionEx)(
        MIDI_event_t* evt,
        KbdSectionInfo* section,
        HWND hwndCtx);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*kbd_RunCommandThroughHooks)(
        KbdSectionInfo* section,
...
REAPERAPI_DEF //==============================================
    ...
    LICE_IBitmap* (*LICE_LoadPNGFromResource)(
        HINSTANCE hInst,
        const char* resid,
        LICE_IBitmap* bmp);
#endif
...
REAPERAPI_DEF //==============================================
              // LICE_ThickFLine
    // always AA. wid is not affected by scaling (1 is always normal line, 2 is
    // always 2 physical pixels, etc)

    void (*LICE_ThickFLine)(
        LICE_IBitmap* dest,
        double x1,
        double y1,
        double x2,
...
REAPERAPI_DEF //==============================================
    ...
    double (
        *MIDI_GetPPQPos_StartOfMeasure)(MediaItem_Take* take, double ppqpos);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Gets a recent MIDI input event from the global history. idx=0 for the
    // most recent event, which also latches to the latest MIDI event state (to
    // get a more recent list, calling with idx=0 is necessary). idx=1 next most
    // recent event, returns a non-zero sequence number for the event, or zero
    // if no more events. tsOut will be set to the timestamp in samples relative
    // to the current position (0 is current, -48000 is one second ago, etc).
    // devIdxOut will have the low 16 bits set to the input device index, and
    // 0x10000 will be set if device was enabled only for control. projPosOut
    // will be set to project position in seconds if project was playing back at
    // time of event, otherwise -1. Large SysEx events will not be included in
    // this event list.

    int (*MIDI_GetRecentInputEvent)(
        int idx,
        char* bufOut,
        int* bufOut_sz,
        int* tsOut,
...
REAPERAPI_DEF //==============================================
    ...
    // force_reinit_output force a particular device index to close/re-open
    // (pass -1 to not force any devices to reopen).
...
REAPERAPI_DEF //==============================================
    ...
    // list the takes that are currently being edited in this MIDI editor,
    // starting with the active take. See MIDIEditor_GetTake

        MediaItem_Take* (*MIDIEditor_EnumTakes)(
            HWND midieditor,
            int takeindex,
            bool editable_only);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // (0=C) list_cnt: if viewing list view, returns event count if setting_desc
    // is unsupported, the function returns -1. See MIDIEditor_SetSetting_int,
    // MIDIEditor_GetActive, MIDIEditor_GetSetting_str
    //
...
REAPERAPI_DEF //==============================================
    ...
    // scale list_X: if viewing list view, returns string describing event at
    // row X (0-based). String will have a list of key=value pairs, e.g.
    // 'pos=4.0 len=4.0 offvel=127 msg=90317F'. pos/len times are in QN,
    // len/offvel may not be present if event is not a note. other keys which
    // may be present include pos_pq/len_pq, sel, mute, ccval14, ccshape,
    // ccbeztension. if setting_desc is unsupported, the function returns false.
    // See MIDIEditor_GetActive, MIDIEditor_GetSetting_int
    //

    bool (*MIDIEditor_GetSetting_str)(
        HWND midieditor,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*MIDIEditor_LastFocused_OnCommand)(
        int command_id,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*MIDIEditor_SetSetting_int)(
        HWND midieditor,
        const char* setting_desc,
        int setting);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    double (
        *parse_timestr_len)(const char* buf, double offset, int modeoverride);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    PCM_sink* (*PCM_Sink_CreateMIDIFileEx)(
        ReaProject* proj,
        const char* filename,
        const char* cfg,
        int cfg_sz,
...
REAPERAPI_DEF //==============================================
              ...
              // Calls and returns PCM_source::PeaksBuild_Begin() if mode=0,
    // PeaksBuild_Run() if mode=1, and PeaksBuild_Finish() if mode=2. Normal use
    // is to call PCM_Source_BuildPeaks(src,0), and if that returns nonzero,
    // call PCM_Source_BuildPeaks(src,1) periodically until it returns zero (it
    // returns the percentage of the file remaining), then call
    // PCM_Source_BuildPeaks(src,2) to finalize. If PCM_Source_BuildPeaks(src,0)
    // returns zero, then no further action is necessary.
...
REAPERAPI_DEF //==============================================
    ...
    // imported as in-project MIDI events.
        ...
        PCM_source* (*PCM_Source_CreateFromFileEx)(
            const char* filename,
            bool forcenoMidiImp);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*PCM_Source_GetSectionInfo)(
        PCM_source* src,
        double* offsOut,
        double* lenOut,
        bool* revOut);
...
REAPERAPI_DEF //==============================================
    ...
    REAPER_PeakBuild_Interface* (*PeakBuild_CreateEx)(
        PCM_source* src,
        const char* fn,
        int srate,
        int nch,
...
REAPERAPI_DEF //==============================================
              // realloc_cmd_clear
              // clears a buffer/buffer-size registration added with
    // realloc_cmd_register_buf, and clears any later registrations, frees any
    // allocated buffers. call after values are read from any registered
    // pointers etc.
...
#if defined(REAPERAPI_WANT_realloc_cmd_register_buf) || \
    !defined(REAPERAPI_MINIMAL)
REAPERAPI_DEF //==============================================
              // realloc_cmd_register_buf
    // registers a buffer/buffer-size which may be reallocated by an API
    // (ptr/ptr_size will be updated to the new values). returns a token which
    // should be passed to realloc_cmd_clear after API call and values are read.
...
REAPERAPI_DEF //==============================================
    ...
    // finishes. If called within a Lua context and errmsg has a ! prefix,
...
REAPERAPI_DEF //==============================================
    ...
    // Resolve a wildcard pattern into a set of nul-separated, double-nul
    // terminated render target filenames. Returns the length of the string
    // buffer needed for the returned file list. Call with path=NULL to suppress
    // filtering out illegal pathnames, call with targets=NULL to get just the
    // string buffer length.

    int (*ResolveRenderPattern)(
        ReaProject* project,
        const char* path,
        const char* pattern,
        char* targets,
...
REAPERAPI_DEF //==============================================
    ...
    void (*SendLocalOscMessage)(
        void* local_osc_handler,
        const char* msg,
        int msglen);
#endif

#if defined(REAPERAPI_WANT_SendMIDIMessageToHardware) || \
    !defined(REAPERAPI_MINIMAL)
REAPERAPI_DEF //==============================================
              // SendMIDIMessageToHardware
    // Sends a MIDI message to output device specified by output. Message is
    // sent in immediate mode.
...
REAPERAPI_DEF //==============================================
    ...
    // set &1 to show the master track in the TCP, &2 to HIDE in the mixer.
...
REAPERAPI_DEF //==============================================
    ...
    // * : free item positioning Y-position, 0=top of track, 1=bottom of track
    // (will never be 1) F_FREEMODE_H : float * : free item positioning height,
    // 0=no height, 1=full height of track (will never be 0)
    //
...
REAPERAPI_DEF //==============================================
    ...
    // mouse modifier behavior to factory default. Note: the action string may
    // have a space and 'c' or 'm' appended to it to specify command ID vs mouse
    // modifier ID. Special case: SetMouseModifier(context, -1, -1) will reset
    // the entire context to default. Special case: SetMouseModifier(-1, -1, -1)
    // will reset all contexts to default. See GetMouseModifier.
    //

    void (*SetMouseModifier)(
        const char* context,
        int modifier_flag,
        const char* action);
#endif
...
REAPERAPI_DEF //==============================================
              // SetProjectMarker
    // Note: this function can't clear a marker's name (an empty string will
    // leave the name unchanged), see SetProjectMarker4.

    bool (*SetProjectMarker)(
        int markrgnindexnumber,
        bool isrgn,
        double pos,
        double rgnend,
...
REAPERAPI_DEF //==============================================
    ...
    // flags&1 to clear name. If flags&2, markers will not be re-sorted, and
    // after making updates, you MUST call SetProjectMarkerByIndex2 with
    // markrgnidx=-1 and flags&2 to force re-sort/UI updates.

    bool (*SetProjectMarkerByIndex2)(
        ReaProject* proj,
        int markrgnidx,
        bool isrgn,
        double pos,
...
REAPERAPI_DEF //==============================================
    ...
    // Add (flag > 0) or remove (flag < 0) a track from this region when using
    // the region render matrix. If adding, flag==2 means force mono, flag==4
    // means force stereo, flag==N means force N/2 channels.

    void (*SetRegionRenderMatrix)(
        ReaProject* proj,
        int regionindex,
        MediaTrack* track,
        int flag);
...
REAPERAPI_DEF //==============================================
              ...
              // ColorToNative(r,g,b). To unset the track color, see
              // SetMediaTrackInfo_Value I_CUSTOMCOLOR
...
REAPERAPI_DEF //==============================================
    ...
    // 1 beat 2\t2.1.1\tLyric for measure 2 beat 1	"). See GetTrackMIDILyrics
...
REAPERAPI_DEF //==============================================
    ...
    // otherwise stereo pair, hwout:&512=rearoute I_MIDIFLAGS : int * : low 5
    // bits=source channel 0=all, 1-16, next 5 bits=dest channel, 0=orig,
    // 1-16=chan See CreateTrackSend, RemoveTrackSend, GetTrackNumSends.
...
REAPERAPI_DEF //==============================================
    ...
    bool (*SetTrackStateChunk)(
        MediaTrack* track,
        const char* str,
        bool isundoOptional);
#endif

#if defined(REAPERAPI_WANT_SetTrackUIInputMonitor) || \
    !defined(REAPERAPI_MINIMAL)
REAPERAPI_DEF //==============================================
              // SetTrackUIInputMonitor
    // monitor: 0=no monitoring, 1=monitoring, 2=auto-monitoring. returns new
    // value or -1 if error. igngroupflags: &1 to prevent track grouping, &2 to
    // prevent selection ganging

    int (*SetTrackUIInputMonitor)(
        MediaTrack* track,
        int monitor,
        int igngroupflags);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // error. igngroupflags: &1 to prevent track grouping, &2 to prevent
    // selection ganging
...
REAPERAPI_DEF //==============================================
              // SetTrackUIPan
    // igngroupflags: &1 to prevent track grouping, &2 to prevent selection
    // ganging

    double (*SetTrackUIPan)(
        MediaTrack* track,
        double pan,
        bool relative,
        bool done,
...
REAPERAPI_DEF //==============================================
              // SetTrackUIPolarity
    // polarity (AKA phase): <0 toggles, 0=normal, >0=inverted. returns new
    // value or -1 if error.igngroupflags: &1 to prevent track grouping, &2 to
    // prevent selection ganging

    int (*SetTrackUIPolarity)(
        MediaTrack* track,
        int polarity,
        int igngroupflags);
#endif
...
REAPERAPI_DEF //==============================================
              // SetTrackUIRecArm
    // recarm: <0 toggles, >0 sets recarm, 0=unsets recarm. returns new value or
...
REAPERAPI_DEF //==============================================
    ...
    // src_take=dest_take to reorder.

    void (*TakeFX_CopyToTake)(
...
REAPERAPI_DEF //==============================================
    ...
    bool (*TakeFX_FormatParamValueNormalized)(
        MediaItem_Take* take,
        int fx,
        int param,
        double value,
...
REAPERAPI_DEF //==============================================
    ...
    // create=true, the envelope will be created.
        ...
        TrackEnvelope* (*TakeFX_GetEnvelope)(
            MediaItem_Take* take,
            int fxindex,
            int parameterindex,
            bool create);
...
REAPERAPI_DEF //==============================================
    ...
    bool (*TakeFX_GetFormattedParamValue)(
        MediaItem_Take* take,
        int fx,
        int param,
        char* bufOut,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*TakeFX_GetNamedConfigParm)(
        MediaItem_Take* take,
        int fx,
        const char* parmname,
        char* bufOutNeedBig,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*TakeFX_GetParameterStepSizes)(
        MediaItem_Take* take,
        int fx,
        int param,
        double* stepOut,
...
#if defined(REAPERAPI_WANT_TakeFX_GetParamFromIdent) || \
    !defined(REAPERAPI_MINIMAL)
REAPERAPI_DEF //==============================================
...
REAPERAPI_DEF //==============================================
    ...
    double (
        *TakeFX_GetParamNormalized)(MediaItem_Take* take, int fx, int param);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*TakeFX_GetUserPresetFilename)(
        MediaItem_Take* take,
        int fx,
        char* fnOut,
        int fnOut_sz);
...
REAPERAPI_DEF //==============================================
    ...
    bool (
        *TakeFX_NavigatePresets)(MediaItem_Take* take, int fx, int presetmove);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // gets plug-in specific named configuration value (returns true on
    // success). see TrackFX_SetNamedConfigParm

    bool (*TakeFX_SetNamedConfigParm)(
        MediaItem_Take* take,
        int fx,
        const char* parmname,
        const char* value);
...
REAPERAPI_DEF //==============================================
    ...
    bool (
        *TakeFX_SetParam)(MediaItem_Take* take, int fx, int param, double val);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*TakeFX_SetParamNormalized)(
        MediaItem_Take* take,
        int fx,
        int param,
        double value);
...
REAPERAPI_DEF //==============================================
    ...
    const char* (*ThemeLayout_GetParameter)(
        int wp,
...
REAPERAPI_DEF //==============================================
    ...
    int (*TimeMap_GetMetronomePattern)(
        ReaProject* proj,
        double time,
        char* pattern,
        int pattern_sz);
...
REAPERAPI_DEF //==============================================
              // Track_GetPeakHoldDB
    // Returns meter hold state, in dB*0.01 (0 = +0dB, -0.01 = -1dB, 0.02 =
    // +2dB, etc). If clear is set, clears the meter hold. If channel==1024 or
    // channel==1025, returns loudness values if this is the master track or
    // this track's VU meters are set to display loudness.
...
REAPERAPI_DEF //==============================================
              // Track_GetPeakInfo
              // Returns peak meter value (1.0=+0dB, 0.0=-inf) for channel. If
    // channel==1024 or channel==1025, returns loudness values if this is the
    // master track or this track's VU meters are set to display loudness.
...
REAPERAPI_DEF //==============================================
    ...
    bool (*TrackFX_FormatParamValueNormalized)(
        MediaTrack* track,
        int fx,
        int param,
        double value,
...
REAPERAPI_DEF //==============================================
    ...
    // 4=hishelf, 5=lopass, 6=bandpass, 7=parallel bandpass. Bandidx (ignored
    // for master gain): 0=target first band matching bandtype, 1=target 2nd
    // band matching bandtype, etc.
    //
    // See TrackFX_GetEQ, TrackFX_GetEQParam, TrackFX_SetEQParam,
    // TrackFX_SetEQBandEnabled.

    bool (*TrackFX_GetEQBandEnabled)(
        MediaTrack* track,
        int fxidx,
        int bandtype,
        int bandidx);
...
REAPERAPI_DEF //==============================================
    ...
    // 4=hishelf, 5=lopass, 6=bandpass, 7=parallel bandpass. Bandidx (ignored
    // for master gain): 0=target first band matching bandtype, 1=target 2nd
    // band matching bandtype, etc. Paramtype (ignored for master gain): 0=freq,
    // 1=gain, 2=Q. See TrackFX_GetEQ, TrackFX_SetEQParam,
    // TrackFX_GetEQBandEnabled, TrackFX_SetEQBandEnabled.

    bool (*TrackFX_GetEQParam)(
        MediaTrack* track,
        int fxidx,
        int paramidx,
        int* bandtypeOut,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*TrackFX_GetFormattedParamValue)(
        MediaTrack* track,
        int fx,
        int param,
        char* bufOut,
...
REAPERAPI_DEF //==============================================
    ...
    // param.X.learn.[midi1,midi2,osc] : first two bytes of MIDI message, or OSC
    // string if set param.X.learn.mode : absolution/relative mode flag (0:
    // Absolute, 1: 127=-1,1=+1, 2: 63=-1, 65=+1, 3: 65=-1, 1=+1, 4: toggle if
    // nonzero) param.X.learn.flags : &1=selected track only, &2=soft takeover,
    // &4=focused FX only, &8=LFO retrigger, &16=visible FX only BANDTYPEx,
    // BANDENABLEDx : band configuration [ReaEQ] THRESHOLD, CEILING, TRUEPEAK :
    // [ReaLimit] NUMCHANNELS, NUMSPEAKERS, RESETCHANNELS : [ReaSurroundPan]
    // ITEMx : [ReaVerb] state configuration line, when writing should be
    // followed by a write of DONE FILE, FILEx, -FILEx, +FILEx, -FILE* : [RS5k]
    // file list, -/+ prefixes are write-only, when writing any, should be
    // followed by a write of DONE MODE, RSMODE : [RS5k] general mode, resample
    // mode VIDEO_CODE : [video processor] code force_auto_bypass : 0 or 1 -
    // force auto-bypass plug-in on silence instance_oversample_shift : instance
    // oversampling shift amount, 0=none, 1=~96k, 2=~192k, etc. When setting
    // requires playback stop/start to take effect chain_oversample_shift :
    // chain oversampling shift amount, 0=none, 1=~96k, 2=~192k, etc. When
    // setting requires playback stop/start to take effect chain_pdc_mode :
    // chain PDC mode (0=classic, 1=new-default, 2=ignore PDC, 3=hwcomp-master)
    //
...
REAPERAPI_DEF //==============================================
    ...
    // gets the parameter index from an identifying string (:wet, :bypass,
    // :delta, or a string returned from GetParamIdent), or -1 if unknown.

    int (*TrackFX_GetParamFromIdent)(
        MediaTrack* track,
        int fx,
        const char* ident_str);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*TrackFX_GetUserPresetFilename)(
        MediaTrack* track,
        int fx,
        char* fnOut,
        int fnOut_sz);
...
REAPERAPI_DEF //==============================================
    ...
    // bandtype. Bandtype: -1=master gain, 0=hipass, 1=loshelf, 2=band, 3=notch,
    // 4=hishelf, 5=lopass, 6=bandpass, 7=parallel bandpass. Bandidx (ignored
    // for master gain): 0=target first band matching bandtype, 1=target 2nd
    // band matching bandtype, etc. Paramtype (ignored for master gain): 0=freq,
    // 1=gain, 2=Q. See TrackFX_GetEQ, TrackFX_GetEQParam,
    // TrackFX_GetEQBandEnabled, TrackFX_SetEQBandEnabled.

    bool (*TrackFX_SetEQParam)(
        MediaTrack* track,
        int fxidx,
        int bandtype,
        int bandidx,
...
REAPERAPI_DEF //==============================================
    ...
    bool (
        *TrackFX_SetPreset)(MediaTrack* track, int fx, const char* presetname);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (
        *ValidatePtr2)(ReaProject* proj, void* pointer, const char* ctypename);
#endif
...
#if defined(REAPERAPI_WANT_GetFreeDiskSpaceForRecordPath) || \
    !defined(REAPERAPI_MINIMAL)
        {(void**)&GetFreeDiskSpaceForRecordPath,
         "GetFreeDiskSpaceForRecordPath"},
#endif
#if defined(REAPERAPI_WANT_GetFXEnvelope) || !defined(REAPERAPI_MINIMAL)
...
#if defined(REAPERAPI_WANT_MIDI_GetPPQPos_StartOfMeasure) || \
    !defined(REAPERAPI_MINIMAL)
        {(void**)&MIDI_GetPPQPos_StartOfMeasure,
         "MIDI_GetPPQPos_StartOfMeasure"},
#endif
#if defined(REAPERAPI_WANT_MIDI_GetPPQPosFromProjQN) || \
    !defined(REAPERAPI_MINIMAL)
...
#if defined(REAPERAPI_WANT_TakeFX_GetFormattedParamValue) || \
    !defined(REAPERAPI_MINIMAL)
        {(void**)&TakeFX_GetFormattedParamValue,
         "TakeFX_GetFormattedParamValue"},
#endif
#if defined(REAPERAPI_WANT_TakeFX_GetFXGUID) || !defined(REAPERAPI_MINIMAL)
...
#if defined(REAPERAPI_WANT_TrackFX_GetParameterStepSizes) || \
    !defined(REAPERAPI_MINIMAL)
        {(void**)&TrackFX_GetParameterStepSizes,
         "TrackFX_GetParameterStepSizes"},
#endif
#if defined(REAPERAPI_WANT_TrackFX_GetParamEx) || !defined(REAPERAPI_MINIMAL)
