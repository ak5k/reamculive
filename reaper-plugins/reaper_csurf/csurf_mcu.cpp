...
class CSurf_MCU : public IReaperControlSurface {
    ...
    unsigned int m_fader_lasttouch[256]; // m_fader_touchstate changes will
...
#define FADER_REPOS_WAIT 250
    ...
    int GetBankOffset() const
    {
        return (m_cfg_flags & CONFIG_FLAG_NOBANKOFFSET)
                   ? (m_offset + 1)
                   : (m_offset + 1 + m_allmcus_bank_offset);
    }
    ...
    bool OnBankChannel(MIDI_event_t* evt)
    {
        ...
        if (evt->midi_message[1] & 1) // increase by X
        {
            ...
            if (movesize > 1) {
                if (m_allmcus_bank_offset + maxfaderpos >= msize)
                    return true;
            }

            m_allmcus_bank_offset += movesize;

            if (m_allmcus_bank_offset >= msize)
                m_allmcus_bank_offset = msize - 1;
        }
        else {
            m_allmcus_bank_offset -= movesize;
            if (m_allmcus_bank_offset < 0)
                m_allmcus_bank_offset = 0;
        }
    ...
    bool OnButtonPress(MIDI_event_t* evt)
    {
        ...
        static const ButtonHandler handlers[nHandlers] = {
            ...
            {0x18,
             0x1f,
             &CSurf_MCU::OnChannelSelect,
             &CSurf_MCU::OnChannelSelectDC},
	  ...
	  UpdateMackieDisplay( 0, buf, 56 );
        ...
        for (int i = nPressOnlyHandlers; i < nHandlers; i++)
            if (handlers[i].evt_min <= evt_code &&
...
#define FIXID(id)                                              \
    ...
    void SetSurfacePan(MediaTrack* trackid, double pan)
    {
        ...
        if (m_midiout && id >= 0 && id < 256 && id < m_size) {
            ...
            if (m_flipmode) {
                ...
                int panint = volToInt14(GetSendLevel(trackid));
    ...
    virtual int Extended(int call, void* parm1, void* parm2, void* parm3)
    {
        ...
        if (call == CSURF_EXT_SETSENDVOLUME && m_flipmode) {
